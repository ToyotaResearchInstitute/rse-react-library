    ...
    <div className="w-[264px] p-3">
      <div className="mb-2.5 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button type="button" aria-label="Previous month" onClick={() => setViewDate(new Date(year, month - 1, 1))} className="inline-flex size-6 items-center justify-center rounded-md text-neutral-600 hover:bg-muted">
            <ChevronLeft className="size-3.5" />
          </button>
          <button type="button" aria-label="Next month" onClick={() => setViewDate(new Date(year, month + 1, 1))} className="inline-flex size-6 items-center justify-center rounded-md text-neutral-600 hover:bg-muted">
      ...
      <div className="mb-1 grid grid-cols-7 gap-0.5">
        {WEEKDAYS.map((d, i) => (
          <div key={i} className="py-1 text-center font-mono text-[10px] font-semibold uppercase text-muted-foreground">{d}</div>
        ))}
      </div>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent align="start" className="w-auto p-0">
          <div className="flex">
            ...
            <div>
              <RangeMonthGrid range={draft} onSelect={setDraft} viewDate={viewDate} setViewDate={setViewDate} />
              <div className="flex justify-end gap-2 border-t border-border px-3 py-2.5">
                <Button variant="ghost" size="sm" onClick={() => { setDraft({}); onValueChange?.({}); setOpen(false) }}>
                  Clear
                </Button>
